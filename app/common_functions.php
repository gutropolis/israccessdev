		 ...
		 if(count($event_list) > 0 ){
				 ...
				 foreach($event_list as $get) {
					 ...
					 $array_data = array();
							...
							$categoryid=App\Models\EventGroupCategory::select('category_id')->where('events_group_id',$get['id'])->first();
							if(count($categoryid)>0)
							{
								$eventgrpcatid=App\Models\Category::where('id',$categoryid['category_id'])->first();
							}
							else
							{
...
function sendEmailOrder($from='',$to,$subject,$msgArr, $emailtemplate,$pdfcontent){
	...
	function sendEmailNews($from='',$to,$subject,$msgArr, $emailtemplate){
		...
		//Mail Jet create contact
		
		function createContact($Cemail)
		{
			//echo $Cemail;
			$apiKey='b36d66514a5568fa15cccc5b76f4d4b1';
			$secretKey='e629758e6fb99661fdf7784c27d1aa0f';
