...
class AdminOrderController extends Base 
{
	...
	public function getAjaxOrdersList($request, $response){
		...
		if($isFiltered){
			...
			if ($result_data = mysqli_query($this->conn,$sqlQuery))
			  {
			     ...
			     while($get = mysqli_fetch_object($result_data)){
					...
					$user_id = (int) $get->customer_id;
					// Get Seat Sequence here
					$orderitem = Models\OrderItems::where('order_id', $get->id)->where('type_product', 'event')->get();
					$array_data['customer_name'] = Models\Usermeta::where('user_id', $user_id)->first()->first_name; //firstname
					$array_data['last_name']  = $get['Customer']['name']; // Last Name
					...
					$array_data['event_name'] =  Models\Event::where('id', $orderitem[0]->product_id)->first()->title; //event name
			...
			foreach($orders_list as $get){
				...
				$array_data['event_name'] =  Models\Event::where('id', $orderitem[0]->product_id)->first()->title;				
				$data[] = $array_data;
			}					   
		}
			...
			if ($result_data = mysqli_query($this->conn,$sqlQuery))
			  {
			     ...
			     while($get = mysqli_fetch_object($result_data)){
					...
					$orderitem = Models\OrderItems::where('order_id', $get->id)->where('type_product', 'event')->get();
					$array_data['customer_name'] = Models\Usermeta::where('user_id', $user_id)->first()->first_name; //firstname
					$array_data['last_name']  = $get->customer_name; // Last Name
					$array_data['total_amount']  = $get->total_amount; // Total Amount
					$array_data['payment_type']  = $get->payment_type; // Payment Type
					$array_data['seat_category']  = $orderitem[0]->ticket_category; // Ticket Category
					$array_data['seat_row']  = $orderitem[0]->ticket_row; // Ticket Row
	...
	public function getOrderById($request, $response, $args){
				 ...
				 if (strpos($seat_ids_sequence, ',') !== false) {
					 ...
					 foreach($seat_ids_sequence as $key=>$seat_id){
						...
						$order_itmes_list  .= '<tr>
									...
									<td>'.$categoryName.'</td>
									<td>'.$rowNumber.'</td>
									<td class="text-center">'.$seatCat['seat_number'].'</td>
		...
		$order_date_only = get_month_name($month_name). date(' j , Y',  strtotime($order['created_on']));
		$order_time_only = date('H:i', strtotime($order['created_on'])); 
		$order_date      = $order_date_only.' '. $order_time_only;
	   ...
	   // get previous user id
		...
		// For specific seats to change
		$few_seats_popup = '';
		$seats_drop_down_list = '';
		...
		if($num_rows > 0){
			...
			while($row = mysqli_fetch_assoc($resultCatRows)){
				$cat_and_row_label = $row['seat_category'].' - '.$row['row_number'];
				$seats_drop_down_list .= '<optgroup label="'.$cat_and_row_label.'" data-max-options="1">'; 
				...
				if($seat_num_rows > 0){
					while($seat = mysqli_fetch_assoc($resultEmptySeats)){
						...
						$seats_drop_down_list .= '<option value="'.$seat['id'].'">'.$seat['seat_number'].'</option>';
					}
					$seats_drop_down_list .= '</optgroup>';
				}else{
		...
		// Get seat quantity
		//$total_seat_quantity  = Models\OrderItems::whereRaw('type_product="event"')->sum('qantity');
									...
									<div class="col-md-6 pull-right">
									 <form id="customer_refund_frm" accept-charset="UTF-8" class="row" style="width:100%">
										...
										<div class="form-item">
										 ...
										 <input type="hidden" name="order_id" value="'.$id.'">
										 <input type="hidden" name="record_id"  value="'.$record_id.'">
										 <button type="button"  id="btn_save_seats_refund" onClick="refundOrder()" class="btn btn-info pull-right" style="margin:0 auto;float:  left;margin-left: 0px;"><i class="fa fa-save"></i> '.$this->lang['order_refund_txt'].' </button> 
										</div>   
									  </form> 
									</div>
								  </div>
		...
		$sqlCoupon = "SELECT C.discount_amount,C.discount_type FROM `coupons` AS C LEFT JOIN
		...
		if($rowCoupon = mysqli_fetch_assoc($resultCoupon)){
			...
			if($discount_type == 'Percentage'){
				$discount_amount = (($sub_total / 100)*$rowCoupon['discount_amount']);
			}elseif($discount_type == "PerTicket"){ //Discount per ticket
				$quantities_all =0;
				$current_order_id = 0;
				foreach($order_itmes_data as $row){
					if($row['type_product'] != 'booking_fees'){
						$quantities_all += $row['quantity'];
						$current_order_id = $row['order_id'];
					}
				}

				$discount_amount = $rowCoupon['discount_amount'] * $quantities_all;
				
			}elseif($discount_type == "Double"){ //one ticket offered
				...
				foreach($order_itmes_data as $row){
					if($row['type_product'] != 'booking_fees'){				
						if(!isset($grouped[$row['ticket_category']])){						
							$grouped[$row['ticket_category']]['quantity'] = $row['seat_qty'];
							$grouped[$row['ticket_category']]['price'] = $row['price'];
						}else{							
							$grouped[$row['ticket_category']]['quantity'] += $row['seat_qty'];
						}
					}					
				...
				foreach($grouped as $group){
					if($group['quantity'] > 1){
						$discount_amount = $discount_amount + (intval($group['quantity'] / 2 ) * ( 	$group['price'] + $bookingfee) ) ;
					}
				}
			}else{
		...
		$params = array( 'title' => $this->lang['order_detail_txt'],
						...
						'customer_email' => $customer_first_name. ' '. $customer[0]['name'] . '<br>'.$customer[0]['email'].' <br>'.$customer[0]['customerdata']['phone_no'],
						...
						'few_seats_popup' => $seats_drop_down_list);
  ...
  public function seatBox($seat_number, $seat_id, $event_id){
	...
	public function changeSeats($request, $response, $args){
	   ...
	   if (strpos($_REQUEST['seat_ids_sequence'], ',') !== false) {
		...
		$newSeatsList = Models\EventCategoryRowSeat::whereIn('id', $newSeatIds)->
		...
		$seat_ids_sequence = $newSeatsList[0]->seat_sequence_ids;	
			...
			// First get all the new seat(s) 
			$newSeatsList = Models\EventCategoryRowSeat::whereIn('id', array($selectedIds))->
							selectRaw('GROUP_CONCAT(seat_number) as seat_sequence')->
							selectRaw('GROUP_CONCAT(id) as seat_sequence_ids')->
							get();
			$seat_sequence = $newSeatsList[0]->seat_sequence;	
	...
	// Save Admin Comments
	public function saveOrderComments($request, $response, $args){
		$admin_comments = $request->getParam('admin_comments');
		$order_id = $request->getParam('order_id');
		$updateOrder = array('admin_comments' => $admin_comments);
		$updateOrderTable = Models\Order::where('id', '=', $order_id)->
			                              update($updateOrder);
		 return $response
            ->withHeader('Content-type','application/json')
	...
	public function getOrderSeats($request, $response, $args){
		$order_id = $args['order_id']; // The order id
		// Get all seats of this order
		$orderSeats = '';
		$sqlQuery = "SELECT * FROM `orderitems` WHERE order_id=".$order_id." AND `type_product`='event'";
		if($resultData = mysqli_query($this->conn,$sqlQuery))
		 {
			// Get order data here
			$orderData = mysqli_fetch_assoc($resultData);  
			$row_id = $orderData['id'];
			$product_id = $orderData['product_id'];
			$ticket_row = $orderData['ticket_row'];
			$quantity = $orderData['quantity'];
			$price = $orderData['price'];
			$ticket_category = $orderData['ticket_category'];
			$seat_qty = $orderData['seat_qty'];
			$event_ticket_category_id = $orderData['event_ticket_category_id'];
			$ticket_row_id = $orderData['ticket_row_id'];
			$seat_from = $orderData['seat_from'];
			$seat_to = $orderData['seat_to'];
			$seat_sequence = $orderData['seat_sequence'];
			$price_sequence = $orderData['price_sequence'];
			$seat_ids_sequence = $orderData['seat_ids_sequence'];
			$OrderSeatHtmlTable = '<style>
								   #tbl_cpn {
							  ...
							  <thead>
								  <th>#</th>
								  <th>'.$this->lang['ordr_crnt_seat_numbr_txt'].'</th>
								  <th>'.$this->lang['ordr_replace_new_seat_number_txt'].'</th>
								  <th>'.$this->lang['ordr_option_txt'].'</th>
							 </thead>';
			// Now check if the seats are more than one
			if (strpos($seat_sequence, ',') !== false) {
				 $seat_sequence = explode(',', $seat_sequence);
				  $seat_ids_sequence = explode(',', $seat_ids_sequence);
				 $i=1;
				 $j=0;
				 foreach($seat_sequence as $key=>$seat){
					$seat_id = $seat_ids_sequence[$j];
					$OrderSeatHtmlTable .= '<tr>
					                       <td>'.$i.'</td>
										   <td>'.$this->seatNumberBox($seat).'</td>
										   <td>'.$this->geteventAvailableSeats($product_id,$seat_id).'</td>
										   <td><a href="javascript:void();" onClick="changeSingleSeat('.$seat_id.','.$order_id.')" id="changeSeatLabel_'.$seat_id.'">'.$this->lang['ordr_change_txt'].'</a></td>
										   </tr>'; 
				 $i++; $j++; }
			}else{
			   	$OrderSeatHtmlTable .= '<tr>
					                       <td>'.$i.'</td>
										   <td>'.$this->seatNumberBox($seat_sequence,$seat_ids_sequence).'</td>
										   <td>'.$this->geteventAvailableSeats($product_id,$seat_ids_sequence).'</td>
										   <td><a href="javascript:void();" onClick="changeSingleSeat('.$seat_ids_sequence.','.$order_id.')" id="changeSeatLabel_'.$seat_ids_sequence.'">'.$this->lang['ordr_change_txt'].'</a></td>
										   </tr>'; 
			}
			
			$OrderSeatHtmlTable .= '</table>';
			
		 }
		 
		 
		$order_id_hidden = '<input type="hidden" name="order_id" value="'.$order_id.'">';
		
		$responseData =  json_encode(array('title' => $this->lang['ordr_chnage_spc_seat_txt'],  
										 'order_seats' => $OrderSeatHtmlTable.$order_id_hidden
										 ));
	   return $response
            ->withHeader('Content-type','application/json')
            ->write($responseData);
	...
	// Get available seats of an event
	public function geteventAvailableSeats($event_id, $seat_id){
	   // Get all seats of this event 
		$sqlCatRows = "SELECT ESC.`event_id`, ESC.id AS category_id, ESC.seat_category, R.id as row_seat_id,
		                R.row_number FROM 
						`event_seat_categories` AS ESC INNER JOIN `row_seats` AS R 
	    ...
	    $num_rows = mysqli_num_rows($resultCatRows);
		$seats_drop_down_list = '<select class="form-control" name="seat_number_drop_'.$seat_id.'" id="seat_number_drop_'.$seat_id.'">';
		if($num_rows > 0){
			while($row = mysqli_fetch_assoc($resultCatRows)){
				$cat_and_row_label = $row['seat_category'].' - '.$row['row_number'];
				$seats_drop_down_list .= '<option  value="">Select Seat</option>'; 
				$seats_drop_down_list .= '<optgroup label="'.$cat_and_row_label.'" data-max-options="1">'; 
				// Get all available seats of this category and row
				$sqlEmptySeats = "SELECT 
								  ECRS.id,
				...
				if($seat_num_rows > 0){
				   ...
				   $seats_drop_down_list .= ''; //'<option value="">'.$this->lang['order_no_seats_txt'].'.</option>';
				}
				//$available_seats .= '';		
			...
			$seats_drop_down_list = '</select>';
		}
		return $seats_drop_down_list;	
	}
	
	
	// Function to display available seats
  public function seatNumberBox($seat_number){
	    $seat_Box  = '<div class="col-md-1" style="margin-left:  25px;">';
		$seat_Box .= '<div class="m-demo__preview m-demo__preview--badge" >';
		$seat_Box .= '<span class="m-badge m-badge--success">'.$seat_number.'</span>';
		$seat_Box .= '</div>';
		$seat_Box .= '</div>';  
	 return $seat_Box;		
  ...
  // Replace order seat
  public function replaceOrderSeats($request, $response, $args){
	  $order_id = $args['order_id']; // Order ID
	  $current_seat_id = $args['current_seat_id']; // Current Seat ID
	  $new_seat_id = $args['new_seat_id']; // New Seat ID
	  // Get customer id
	  $customer_id = Models\Order::where('id', '=', $order_id)->first()->customer_id;
	  // Get old seat number
	  $oldSeatNumber = Models\EventCategoryRowSeat::where('id', '=', $current_seat_id)->first()->seat_number;
	  // Get old seat Price
	  $oldSeatPrice = Models\EventCategoryRowSeat::where('id', '=', $current_seat_id)->first()->seat_price;
	  // Get new seat number
	  $newSeatNumber = Models\EventCategoryRowSeat::where('id', '=', $new_seat_id)->first()->seat_number;
	  // Get new seat Price
	  $newSeatPrice = Models\EventCategoryRowSeat::where('id', '=', $new_seat_id)->first()->seat_price;
	  // Get order data
	  $sqlQuery = "SELECT * FROM `orderitems` WHERE order_id=".$order_id." AND `type_product`='event'";
	 if($resultData = mysqli_query($this->conn,$sqlQuery))
	 {
		...
		$seat_ids_sequence = $orderData['seat_ids_sequence'];
		if (strpos($seat_sequence, ',') !== false) {
				$new_seatsString = $this->makeString($oldSeatNumber, $newSeatNumber, $seat_sequence);
				$new_seatidsString = $this->makeString($current_seat_id, $new_seat_id, $seat_ids_sequence);
				$new_priceSequence = $this->makeString($oldSeatPrice, $newSeatPrice, $price_sequence);
		}else{
		    $new_seatsString = $newSeatNumber;
			$new_seatidsString = $new_seat_id;
			$new_priceSequence = $newSeatPrice;
		}
		// Update the orderitems table
	    $newSeatsArray = array('seat_sequence' => $new_seatsString, 
		                       'seat_ids_sequence' => $new_seatidsString/*,
							   'price_sequence' => $new_priceSequence*/);
	    $updateNewData = Models\OrderItems::where('id', '=', $row_id)->
			                                update($newSeatsArray);	
		// Update the history table
		$seat_changed_reason = 'Admin changed this seat on '. date('d/m/Y H:i');
		$this->changeReservedSeat($current_seat_id, $new_seat_id, $customer_id, $seat_changed_reason);
		 return $response
            ->withHeader('Content-type','application/json')
            ->write(json_encode(array('status' => TRUE)));
	 }
	  ...
	  // Make Seats order + Seat ids order
	  public function makeString($oldSeat, $replaceWith, $seats){
		$replaced =  str_replace($oldSeat, $replaceWith, $seats);
		$seatsArray = explode(',', $replaced);
		sort($seatsArray);
		$Arraylength = count($seatsArray);
		$newSeatsArray = array();
		for($x=0;$x<$Arraylength;$x++)
		{
		  $newSeatsArray[] = $seatsArray[$x];
		}
		// Return the sorted string
		return implode(',', $newSeatsArray);
	}
