...
class AdminProductorDashboardController extends Base 
{
	...
	public function __construct($container)
	{
	 ...
	 public function dashboard($request, $response) {
		...
		// Get all events of the logged in productor
		$all_events = $all_tickets = $all_sale_amount = 0; 
		// Find total member
		$my_all_event_groups = Models\Eventgroup::where('producer_id', '=', $_SESSION['adminId'])->get();
		if( !$my_all_event_groups->isEmpty() ){
			$event_group_ids = '';
			foreach($my_all_event_groups as $row){
			  $event_group_ids .= $row['id'].',';	
			}
			
			// Get all event ids 
			$all_events = Models\Event::whereIn('eventgroup_id', 
				                   array(rtrim($event_group_ids,',')))->get();
			...
			if( !$all_events->isEmpty() ){
				...
				// Get all event seats
				$all_event_seats = Models\EventSeatCategories::whereIn('event_id', 
				                        array(rtrim($event_ids,',')))->get();
				// Get all tickets						
			}
		...
		$params = array( 'title' => $this->lang['productor_txt'] . ' '. $this->lang['left_menu_dashboard_txt'],
			...
			$reportTable .= '<table class="table table-striped m-table table-bordered">
						  <thead>
							<tr>
							  ...
							  <th>'.$this->lang['dashboard_email_txt'].'</th>
							  <th>'.$this->lang['dashboard_ticket_type_txt'].'</th>
							  <th> '.$this->lang['dashboard_source_txt'].' </th>
