...
class AdminDashboardController extends Base 
{
	...
	public function __construct($container)
	{
	 ...
	 public function dashboard($request, $response) {
		...
		**/
		...
		$is_order_searched =  0;
		$customer_id = 0;  
		$from_date = $to_date = $date_range_order = $customer_id = $suggested_names = '';
		...
		if( isset($_REQUEST['suggested_names']) && !empty($_REQUEST['suggested_names']) ){
			 $suggested_names = $_REQUEST['suggested_names'];  
			   
	   }
	   if( isset($_REQUEST['suggested_name_id']) && !empty($_REQUEST['suggested_name_id']) ){
		    $customer_id = $_REQUEST['suggested_name_id'];
		}else{
		   $customer_id = 0;  
	   }
	...
	// Get group events listing
	public function getGroupEventsListing($request, $response, $args){
		$id = $args['id'];
        $validations = [
            v::intVal()->validate($id)
        ];
		...
		$option = '<option value="0">'.$this->lang['dashboard_select_event_text'].'</option>';
		$option .= $result;
		echo $option;
...
public function downloadSaleReportPDF($request, $response, $args){
	       // echo $args['event_group_id']. ' = '.$args['event_id'];
			...
			$reportTable .= '<table class="table table-striped m-table table-bordered"  style="font-size:10px">
						  <thead>
							<tr>
							  ...
							  <th> '.$this->lang['report_order_amount_txt'].' </th>
							</tr>
						  </thead>
						  <tbody>';		
			if(isset($event_group_id) && isset($event_id) ){
				if( !empty($event_group_id) && !empty($event_id) ){	
					...
					if( $orderReport->isEmpty() ){
					  $reportTable .= '<tr>
										  <th scope="row" colspan="7" class="error"> 
										  <font color="red"><center>'.$this->lang['no_data_found_txt'].'</center></font> 
										  </th>
										</tr>';
				    }else{
					...
					foreach($orderReport as $orderItem){
					  $customer_id = $orderItem['Order']['customer_id'];
					  $order_amount = $orderItem['Order']['total_amount'];
					  $ticket_row_id = $orderItem['ticket_row_id'];
					  $ticket_category = $orderItem['ticket_category'];
					  $ticket_row = $orderItem['ticket_row'];
					  $seat_sequence = $orderItem['seat_sequence'];
				...
				$reportTable .= '<tr>
					                      <td colspan="6" style="text-align:right;"><strong>'.$this->lang['dashboard_total_sale_amount_txt'].'</strong></td>
										  <td ><strong>'.number_format($total_sale).'</strong></td>
										</tr>'; 
					}
...
// Download the order items + matching event seat categories
public function orderCSV($request, $response, $args){
	error_reporting(0);
	set_time_limit(0);
	            $sqlQuery = "SELECT E.date as event_date,OI.order_id, UM.first_name, U.name as customer_name, O.customer_id, O.created_on AS order_date, OI.product_id, ESC.event_id, OI.`event_ticket_category_id`, ESC.id as event_cat_id, OI.ticket_row_id,  RS.id as row_id, OI.seat_from, OI.seat_to, 
							OI.seat_sequence   from orderitems AS OI 
							INNER JOIN event_seat_categories AS ESC ON OI.product_id=ESC.event_id INNER JOIN 
							row_seats AS RS ON RS.event_seat_categories_id=ESC.id
							INNER JOIN orders AS O ON O.id=OI.order_id
                            INNER JOIN users AS U ON U.id=O.customer_id
                            INNER JOIN user_meta AS UM ON U.id=UM.user_id
							INNER JOIN events AS E ON E.id=ESC.event_id
                             WHERE OI.type_product='event' AND O.customer_id <> 1
							GROUP BY OI.id ORDER BY OI.product_id ASC, OI.event_ticket_category_id ASC, OI.ticket_row_id ASC, OI.seat_from ASC";
	            $resultQuery = mysqli_query($this->conn, $sqlQuery);
	            ob_end_clean();
		        header( 'Content-Type: text/csv' );
		        header( 'Content-Disposition: attachment;filename=orders_data_'.time().'.csv');
		        $fp = fopen('php://output', 'w');
				 $titles = array($this->lang['dashboard_order_id_txt'], 
				                $this->lang['dashboard_first_name_txt'],
								$this->lang['seat_map_customer_name_txt'],
								'Customer ID',
								$this->lang['report_event_date_txt'],
								$this->lang['report_order_date_txt'],
				                'Order Event ID', 
								'ESC Event ID',
								...
								'booked Seat From',
								'Booked Seat To',
								'Booked Seat Sequence'
							);
				// This line is very very important to overcome the issue of French Characters in the CSV			
				$titles = array_map("utf8_decode", $titles);			
		        fputcsv($fp, $titles);
				while ($row = mysqli_fetch_assoc($resultQuery)) {
					$data = array($row['order_id'], 
					              $row['first_name'],
								  $row['customer_name'],
								  $row['customer_id'],
  ...
  public function auto_suggestion($request, $response){
	  if(isset($_POST['suggested_names']) && !empty($_POST['suggested_names']))
		{
			...
			$sqlQuery = "SELECT 
							  U.id,
							  CONCAT(U.name, ' ', UM.first_name) AS customer_name 
							FROM
							  `users` AS U 
							  INNER JOIN `user_meta` AS UM 
								ON U.id = UM.user_id 
							WHERE U.name LIKE '%".$search_term."%' 
							  OR UM.`first_name` LIKE '%".$search_term."%' 
							GROUP BY U.`id` 
							ORDER BY `customer_name` ASC  LIMIT 6";
			...
			if(mysqli_num_rows($result) > 0)
			{
				...
				{
					?>
					<div id="auto_suggest_boxes" onclick="clickableSearchResult('<?php echo $row['id'].','.$this->htmltoflash($row['customer_name']); ?>');"><font style="font-family:Verdana, Geneva, sans-serif; font-size:12px;"><?php echo $this->htmltoflash($row['customer_name']); ?></font></div>
					<?php
				}
			}
			else
