...
class AdminSellingController extends Base 
{
	...
	public function getEventbyGroup($request, $response) {
		...
		$events=Event::where('eventgroup_id',$eventgroup_id)->get();
           if(count($events) > 0){
           	$bodytxt.=' <option value="">'.$this->lang['selling_select_txt'].'</option>';
        	foreach ($events as $evgArr) {
					$a= array();
						$bodytxt.=' <option value="'.$evgArr['id'].'">('.$evgArr['id'].') - '.$evgArr['title'].' : '.date('d-m-Y', strtotime($evgArr['date'])).'</option></td>';
	...
	public function selling($request, $response) {
		...
		$data=array();
    	$dataEvgroup=Eventgroup::select('title','id')->orderby('id','asc')->get();
				  ...
				  $ticket_list =  EventSeatCategories::where('event_id', $eventid)->get();	
				   ...
				   if(count( $ticket_list) >  0 ){
					 foreach($ticket_list as $ticket){
											...
											$bodyText .= '<tr> <th scope="row"> '.$ticket['seat_category'].'</th>'; 
													...
													$bodyText .= 			    '<option value="">0</option>';
																					 for($i=1; $i <= $totalSeatAvailable; $i++){
																						 $bodyText .= '<option value="'.$i.','.$ticket['category_price'].'">'.$i.'</option>';
																					 }
													$bodyText .= '
																		</select>
																	</td></tr>';
								...
								$bodyText .= ' <div id="booking_fee"></div><div id="total_items"> </div>';	 					
						}
		...
		public	function ajaxcallRawSeat($request, $response){ 
			   $strList='';
				 ...
				 if(intval($rowseat) > 0){
					...
					$total_qantity = EventCategoryRowSeat::where('row_seats_id', $rowseat)->where('status', 'E')->where('placement', '1')->count();	
			  ...
			  $total_amt=0;
			   ...
			   //  $ticket_type_ids = $request->getParam('ticket_type_id'); 
				...
				/* Start New SeatManagement Code Implementation */
					$seatNArr  =  EventCategoryRowSeat::where('row_seats_id', $row_id)->where('status', 'E')->where('placement', '1')->orderBy('id', 'ASC')->get();	
					if(count($seatNArr) > 0 ){
							$seat_sequence_no='';
							$seat_sequence_price = '';
							...
							if(count($seatPriceSquenceArr) > 0 ){
								  ...
								  $endIndex =$qtx-1;
									...
									for( $i = $startindex; $i<= $endIndex; $i++ ){ 
											...
											$pricehtml.='<span  data-toggle="tooltip" data-placement="top" title="Price: '.$seatPriceSquenceArr[$i].' &#8362;"  class="dot"   ></span>';
									  ...
									  if(count($_SESSION["cart_products"])>0){
											...
											foreach($_SESSION["cart_products"] as $cartItem){	
												...
												$orderitem->save(); //echo 'event'.$orderitem->id;
													...
													update_row_Quantity($row_id,$booked_seats_quantity,$seatSeq); 
