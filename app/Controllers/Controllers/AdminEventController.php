		...
		if( $seats_map->isEmpty() ){
			...
			$i=0;
			 ...
			 foreach($counterRow as $keyInner=>$val){
				...
				$seat_rows_list = Models\EventCategoryRowSeat::with(['Customer'])->where('event_seat_categories_id','=', $row['id'])
				                              ->where('row_seats_id','=', $seatRow['id'])
				...
				$seatStatus  = $rowSeat['status'];
				 ...
				 if($rowSeat['customer_id'] > 0 ){
					...
					$first_Name = Models\Usermeta::where('user_id', '=', $rowSeat['Customer']['id'])->first()->first_name; 	
				   $customer_name = $first_Name. ' '.$rowSeat['Customer']['name'];
				 }else{
				   $customer_name = '';
				 }
				 ...
				 if($seatStatus == 'B'){ // booked
					...
					$ticketStatus = '<a href="javascript:void(0)" class="btn btn-danger m-btn btn-sm 	m-btn m-btn--icon" style="cursor:default">
															<span>
																<i class="fa fa-ban"></i>
																<span>
					 ...
					 $ticketStatus = '<a href="javascript:void(0)" class="btn btn-warning m-btn btn-sm 	m-btn m-btn--icon" style="cursor:default">
															<span>
					 ...
					 $ticketStatus = '<a href="javascript:void(0)" class="btn btn-success btn-sm m-btn 	m-btn m-btn--icon" style="cursor:default">
															<span>
				...
				$seats_list .= '</table>';
				$add_new_seat_table = '';
				if(!$seat_rows_list->isEmpty() ){
				     $add_new_seat_table .= '<table style="float:right">';
