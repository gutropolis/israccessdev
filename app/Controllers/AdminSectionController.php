...
class AdminSectionController extends Base 
{
	...
	// Get Category Event Groups List
	public function getEventGroupsList($request, $response, $args){
		$id = $args['id'];
        $validations = [
            v::intVal()->validate($id)
        ];
		...
		// Get all Event Groups 
		 $mapped_Groups = Models\Eventgroup::where('section',$id)->where('status', '=', 1)->orderBy('section_display_order',  'ASC')->get();
		 $event_groups_array = array();
		 $list = '';
		 foreach($mapped_Groups as $group){
			$id = $group['id'];
			$title = trim(strip_tags(trim(clearString(htmlspecialchars_decode($group['title']))))). ' - '. hr_date($group['date_begin']). ' / '.hr_date($group['date_end']);
			$list .= '<li data-post-id='.$id.'>'.$title.'</li>';
		}
		
		if(empty($list)){
			echo '<p class="no_data_found">No data found.</p>';
		}else{
		   echo $list;
		}
		/*if ($groups) {
			return $response
            ->withHeader('Content-type','application/json')
            ->write(json_encode($groups));
        }*/
		
	}
	
	// Save event groups order
	public function reOrderEventGroups($request, $response){
		
	   $post_order_ids = $request->getParam('post_order_ids');
	   $post_order = isset($post_order_ids) ? $post_order_ids : [];
	   if(count($post_order)>0){
		 for($order_no= 0; $order_no < count($post_order); $order_no++)
		 {
			 $data = array('section_display_order' => ($order_no+1));
		     $update = Models\Eventgroup::where('id', '=', $post_order[$order_no])->update($data);	
		}
		  ...
		  return $response
            ->withHeader('Content-type','application/json')
            ->write(json_encode(array('status' => FALSE))); 
		}
