...
class AdminAuditoriumController extends Base 
{
	...
	*/
	...
	public function SyncFromEvent($request, $response){


		//load from event the auditorium map
		$event_id = $request->getParam('event_id');
		$aud_id = $request->getPAram('aud_id');


		$auditorium_seats_map = Models\EventAuditoriumMap::where('event_id', '=', $event_id)->first();



		if($auditorium_seats_map->id){

			//save it to auditorium table

			$auditorium = Models\Auditorium::where('id' , '=', $aud_id)->update( array(
				'auditorium_key' => $auditorium_seats_map->auditorium_key ,
				'auditorium_map' => $auditorium_seats_map->auditorium_map
			));
			...
			$data = array('status' => 'event not found');
	...
	public function audiJsonMap($request, $response , $param) {

		// event id
		$aud_id = $param['auditorium_id'];



		// fetch event datas by  event id
		$auditoriums = Auditorium::where('id',intval($aud_id))->first();
		// return datas 

		$data['auditorium_id'] = $auditoriums->id;
		$data['auditorium_key'] = $auditoriums->auditorium_key;
		$data['auditorium_map'] = json_decode($auditoriums->auditorium_map, true);
	...
	public function saveAuditoriumDigitalMap($request, $response){
	   ...
	   $aud_id = $request->getParam('auditorium_id');
	   ...
	   $digitalMapTablePKID = Models\Auditorium::where('id', '=', $aud_id)->first()->id;
	   ...
	   if($digitalMapTablePKID){
		   ...
		   $event = Models\Auditorium::where('id', '=', $digitalMapTablePKID)->update($data);	
	...
	public function editDigitalMap($request, $response, $param){

		$id = $param['id'];


		$auditorium = Models\Auditorium::where('id', $id)->first();

		$params = array(
			'title' => $this->lang['seat_map_set_map_txt'],
			'auditorium_name' => $auditorium->name,
			'id' => $id

		);

		return $this->render($response, ADMIN_VIEW.'/Auditorium/audi_digital_map.twig',$params);
	}
