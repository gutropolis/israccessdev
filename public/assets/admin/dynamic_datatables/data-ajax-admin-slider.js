    ...
    init: function() {
        ...
        t = $(".m_datatable_slider").mDatatable({
            ...
            columns: [{
                ...
                field: "picture_caption",
                title: Slider_Caption,
                filterable: !1,
                width: 150,
                template: "{{picture_caption}}"
            },
			{
            ...
            },
			{
            ...
            },
			 {
                ...
                template: function(t, e, a) {
					var edit_lnk = '';
					var del_lnk = '';
					var edit_link = '\t\t\t\t\t\t<a href="javascript:void(0);" class="m-portlet__nav-link btn m-btn m-btn--hover-accent m-btn--icon m-btn--icon-only m-btn--pill" title="'+EDIT+' '+ slider_txt+'" onclick="edit('+[t.id]+')">\t\t\t\t\t\t\t<i class="la la-edit"></i>\t\t\t\t\t\t</a>';
					var del_link = '\t\t\t\t\t\t<a href="javascript:void(0);" onclick="delete_function('+[t.id]+')" class="m-portlet__nav-link btn m-btn m-btn--hover-danger m-btn--icon m-btn--icon-only m-btn--pill" title="'+DELETE+' '+slider_txt+'">\t\t\t\t\t\t\t<i class="la la-trash"></i>\t\t\t\t\t\t</a>\t\t\t\t\t';
					if(IS_SLIDER_EDIT == 'Y'){
...
jQuery(document).ready(function() {
  if(IS_SLIDER_ADD == 'Y' || IS_SLIDER_EDIT == 'Y' || IS_SLIDER_DEL == 'Y'){								
...
function addSlider(){
	...
	if($("#picture_caption").val()=="")
	{
		$(div_id_or_class).show().addClass('alert alert-danger').html(slider_caption_msg_txt);
		hideErrorDiv(div_id_or_class);
		$("#picture_caption").focus();
		return false;
	}
...
{   
    ...
    $.ajax({
        ...
        success: function(data)
        {
            ...
            $('#id_e').val(data['id']);
			$('#picture_caption_e').val(data['picture_caption']);
...
function updateSlider(){
	...
	if($("#picture_caption_e").val()=="")
	{
		$(div_id_or_class).show().addClass('alert alert-danger').html(slider_caption_msg_txt);
		hideErrorDiv(div_id_or_class);
		$("#picture_caption_e").focus();
		return false;
	}
...
function delete_function(id){
   $('.del_title').html(DELETE+' '+ slider_txt);
   $('.del_text').html(common_delete_confirm_msg_txt+' '+slider_txt+'?');	
   $('#id').val(id);
