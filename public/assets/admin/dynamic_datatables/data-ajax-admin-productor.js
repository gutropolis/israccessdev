            ...
            },
			{
			...
			
			 {
                ...
                template: function(t, e, a) {
					var edit_lnk = '';
					var del_lnk = '';
					var edit_link = '\t\t\t\t\t\t<a href="javascript:void(0);" class="m-portlet__nav-link btn m-btn m-btn--hover-accent m-btn--icon m-btn--icon-only m-btn--pill" title="'+EDIT+' '+ productor_txt +'" onclick="edit('+[t.id]+')">\t\t\t\t\t\t\t<i class="la la-edit"></i>\t\t\t\t\t\t</a>';
					var del_link = '\t\t\t\t\t\t<a href="javascript:void(0);" onclick="delete_function('+[t.id]+')" class="m-portlet__nav-link btn m-btn m-btn--hover-danger m-btn--icon m-btn--icon-only m-btn--pill" title="'+DELETE+' '+ productor_txt +'">\t\t\t\t\t\t\t<i class="la la-trash"></i>\t\t\t\t\t\t</a>\t\t\t\t\t';
					if(IS_PRODUCTOR_EDIT == 'Y'){
						var edit_lnk = edit_link;
					}
					if(IS_PRODUCTOR_DEL == 'Y'){
...
jQuery(document).ready(function() {
  if(IS_PRODUCTOR_ADD == 'Y' || IS_PRODUCTOR_EDIT == 'Y' || IS_PRODUCTOR_DEL == 'Y'){									
...
{   
    ...
    $.ajax({
        ...
        success: function(data)
        {
			...
			if(data['productor_data'] !== 'undefined' && data['productor_data'] !== null && data['productor_data'] != '') {
