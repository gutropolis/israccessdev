            ...
            },
			{
                ...
                template: function(t, e, a) {
					if(t.seats_on_map == 'N'){
						...
						var event_aud_map_link = '\t\t\t\t\t\t<a  href="javascript:void(0);" class="m-portlet__nav-link btn m-btn m-btn--hover-accent m-btn--icon m-btn--icon-only m-btn--pill" title="Manual Seat Map  is not allowed" onclick="no_set_map('+[t.id]+')" >\t\t\t\t\t\t\t<i class="la la-ban" style="color:red"></i>\t\t\t\t\t\t</a>\t\t\t\t\t';
					}
					var event_aud_map_link = '';
                    return '\t\t\t\t\t\t<a style="display:none"  href="javascript:void(0);" class="m-portlet__nav-link btn m-btn m-btn--hover-accent m-btn--icon m-btn--icon-only m-btn--pill" title="View Event" onclick="view('+[t.id]+')" >\t\t\t\t\t\t\t<i class="la la-eye"></i>\t\t\t\t\t\t</a>\t\t\t\t\t\t<a href="javascript:void(0);"  class="m-portlet__nav-link btn m-btn m-btn--hover-accent m-btn--icon m-btn--icon-only m-btn--pill" title="Edit Event" onclick="edit_event('+[t.id]+')">\t\t\t\t\t\t\t<i class="la la-edit"></i>\t\t\t\t\t\t</a>\t\t\t\t\t\t<a href="javascript:void(0);" onclick="delete_function_event('+[t.id]+')" class="m-portlet__nav-link btn m-btn m-btn--hover-danger m-btn--icon m-btn--icon-only m-btn--pill" title="Delete Event" style="display:none">\t\t\t\t\t\t\t<i class="la la-trash"></i>\t\t\t\t\t\t</a>\t\t\t\t\t'+event_aud_map_link
                }
            }]
...
function updateEventGroup(){
	...
	{
		//$(".date_begin").focus();
		$(div_id_or_class).show().addClass('alert alert-danger').html(event_group_date_begin_msg_txt);
		hideErrorDiv(div_id_or_class);
		return false;
	}
	...
	if($("#m_summernote_1").val()=="")
	{
		...
		$(div_id_or_class).show().addClass('alert alert-danger').html(event_group_desc_msg_txt);
		hideErrorDiv(div_id_or_class);
		return false;
	}
	...
	success: function(data) {
		...
		if(data.status == 'error'){
			...
			hideErrorDiv(div_id_or_class);
			  ...
			  if( $('#status').val() == 1){
				  window.location.href = '../../groups_archived';
			  }else{
			    location.reload();	
			  }
		}
...
{   
    ...
    $.ajax({
        ...
        success: function(data)
        {
			...
			//$('#m_summernote_3_edit').val(data['event']['description']);
