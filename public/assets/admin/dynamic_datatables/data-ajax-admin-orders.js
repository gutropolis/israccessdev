...
function sendData(data){
  ...
  var formData = 'admin_comments='+admin_comments+'&order_id='+order_id;	

  //if(admin_comments.length>0){

    $.ajax({

        url : "../../orders/saveOrderComments",
        ...
        //html: true
			...
			swal(
								  ...
								  'Commentaire sauvegardé avec succès',
...
// Change Sepecific Seats

function specific_seats(order_id){
     ...
     if(order_id != ''){
	   ...
	   var info = 'id=' + order_id;
		...
		url: "../../orders/getOrderSeats/"+order_id,
		...
		success: function(data){ 
		   ...
		   $('#modal-specific-seats').modal('show');

		   $('.md_order_title').html(data.title);

		   $('.md_order_seats_display').html(data.order_seats);
...
function changeSingleSeat(seat_id, order_id){

  var current_seat_id = seat_id;

  var order_id = order_id;

  if($('#seat_number_drop_'+seat_id).val() == ''){

	  alert('Please select new seat.');

	  $('#seat_number_drop_'+seat_id).focus();
   ...
   if(current_seat_id != ''){
	  ...
	  var new_seat_id = $('#seat_number_drop_'+seat_id).val();

	  $.ajax({

		type: "GET",

		url: "../../orders/replaceOrderSeats/"+order_id+'/'+current_seat_id+'/'+new_seat_id,
	 ...
	 alert('Something wrong with this order');
